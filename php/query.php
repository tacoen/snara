...
class Query
{
    ...
    private static function chapterOrder(int $bookId): array
    {
        ...
        $list = $data['chapters'] ?? (is_array($data) && isset($data[0]) ? $data : []);
    ...
    private static function writeSaved(int $bookId, string $slug, string $query, array $results): void
    {
    ...
    public static function search(int $bookId, string $query): array
    {
        ...
        foreach ($existing as $r) {
            if (!empty($r['id'])) $seenIds[$r['id']] = true;
        }

        $docDir = Config::dataDir() . '/' . $bookId;
        if (!is_dir($docDir)) return ['error' => "Book $bookId not found"];

        $files      = glob($docDir . '/*.json') ?: [];
        $newResults = [];

        foreach ($files as $filePath) {
            if (strpos($filePath, '/cache/') !== false || strpos($filePath, '/conf/') !== false) continue;

            $raw = @file_get_contents($filePath);
            if (!$raw) continue;

            $doc = json_decode($raw, true);
            if (!is_array($doc) || empty($doc['article'])) continue;

            $docFilename = $doc['filename'] ?? basename($filePath, '.json');

            foreach ($doc['article'] as $articleItem) {
                if (empty($articleItem['content']) || !is_string($articleItem['content'])) continue;

                $dom = new DOMDocument();
                libxml_use_internal_errors(true);
                $dom->loadHTML(
                    mb_convert_encoding($articleItem['content'], 'HTML-ENTITIES', 'UTF-8'),
                ...
                foreach ($paragraphs as $p) {
                    ...
                    if (stripos($paraText, $query) === false) continue;
    ...
    public static function build(int $bookId, string $query, array $removeIds): array
    {
        if ($bookId <= 0 || $query === '') {
            return ['error' => 'Missing bookId or query'];
        }
        ...
        $kept      = array_values(array_filter($current, function ($r) use ($removeSet) {
