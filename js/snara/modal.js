...
export function closeModal(id) {
  if (id) {
    ...
    const anyOpen =
      document.querySelectorAll("#app-overlay .app-modal:not([hidden])")
        .length > 0;
    if (!anyOpen) {
    ...
    document.getElementById("app-overlay")?.setAttribute("hidden", "");
    document
      .querySelectorAll("#app-overlay .app-modal")
...
export function _modalFooter(additionalHTML = "") {
  return `
    <div class="modal-footer">
      ${additionalHTML.trim()}
      <button class="btn-mini mute" id="modal-cancel">Cancel</button>
      <button class="btn-mini primary" id="modal-save">Save</button>
    </div>`;
}
