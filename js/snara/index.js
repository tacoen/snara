...
export class SnaraIndex {
  ...
  async openBookIndex() {
    ...
    modal.innerHTML = this._shell(
      "book-index-modal",
      "Books",
      this._bookIndexBody("loading")
    );
    openModal("book-index-modal");
    icx.delayreplace("#book-index-modal [data-icon]");
    modal
      .querySelector("#idx-new-book")
      ?.addEventListener("click", () => this._createBook(modal));
    try {
      ...
      modal.querySelector(
        ".idx-body"
      ).innerHTML = `<p class="idx-empty">Could not load books: ${esc(
        e.message
      )}</p>`;
    }
  }
  ...
  _bookIndexBody(state) {
    if (state === "loading")
      return `<p class="idx-empty idx-loading">Loading books…</p>`;
    return "";
  }
  ...
  _bookListHTML(books) {
    ...
    return books
      ...
      <div class="idx-row${this.activeBookId == b.id ? " idx-row-active" : ""}"
           data-book-id="${esc(b.id)}" data-book-title="${esc(
          b.title
        ...
        <span class="idx-row-main">
          ...
          <span class="idx-row-sub">id ${esc(String(b.id))} · ${fmtDate(
          b.mtime
        )}</span>
        </span>
  ...
  async _createBook(modal) {
    ...
    try {
      const data = await postJson(AppConfig.apiPath + "?action=book.create", {
        title,
      });
  ...
  async _createChapter(modal) {
    ...
    btn.textContent = "creating…";
    try {
      await postJson(AppConfig.apiPath + "?action=doc.save", {
        filename,
        bookId,
        meta: {},
        article: [],
      ...
      const [chapters, states] = await Promise.all([
        apiFetch(
          AppConfig.apiPath +
            `?action=book.chapters&id=${encodeURIComponent(bookId)}`
        ),
        this._loadStates(bookId),
      ]);
      const surviving = chapters.filter(
        (ch) => states[ch.filename] !== "delete"
      );
      modal.querySelector(".idx-body").innerHTML =
        this._chapterListHTML(surviving);
      ...
      input.insertAdjacentHTML(
        "afterend",
        `<span style="color:var(--danger);font-size:11px;margin-top:2px">${esc(
          e.message
        )}</span>`
      );
      setTimeout(
        () => modal.querySelector(".idx-toolbar span[style]")?.remove(),
        3000
      );
    }
  }
  ...
  _setActiveBook(id, title) {
    ...
    window.dispatchEvent(
      new CustomEvent("bookchange", {
        detail: {
          bookId: AppConfig.activeBookId,
          title: AppConfig.activeBookTitle,
        },
      })
    );
    ...
    postJson(AppConfig.apiPath + "?action=book.setActive", {
      bookId: id,
    }).catch(() => {});
  }
  ...
  _bindOptMenu(modal) {
    ...
    if (coverLi) {
      coverLi.addEventListener("click", () => {
        ...
        setTimeout(
          () => window.SnaraFiles?.instance?.switchSection?.("gallery"),
          0
        );
      });
    }
    ...
    if (synopsisLi) {
      synopsisLi.addEventListener("click", () => {
        ...
        setTimeout(() => {
          ...
          setTimeout(
            () => window.SnaraFileMan?.instance?._openEditor?.("synopsis.md"),
            80
          );
        }, 0);
      });
  ...
  async openChapterIndex() {
    ...
    modal
      .querySelector("#idx-new-chapter")
      ?.addEventListener("click", () => this._createChapter(modal));
    modal
      .querySelector("#idx-new-chapter-file")
    ...
    try {
      const [chapters, states, hasCover, hasSynopsis] = await Promise.all([
        apiFetch(
          AppConfig.apiPath +
            `?action=book.chapters&id=${encodeURIComponent(this.activeBookId)}`
        ),
        this._loadStates(this.activeBookId),
        this._checkCover(this.activeBookId),
        this._checkAsset(
          `${AppConfig.dataPath}/${this.activeBookId}/files/synopsis.md`
        ),
      ]);
      ...
      modal.querySelector(
        ".idx-body"
      ).innerHTML = `<p class="idx-empty">Could not load chapters: ${esc(
        e.message
      )}</p>`;
    }
...
<div class="idx-row" data-filename="${esc(ch.filename)}"
  ...
  _bindDnd(modal, bookId) {
    ...
    body.querySelectorAll(".idx-row").forEach((row) => {
      ...
      row.addEventListener("dragend", () => {
        ...
        body
          .querySelectorAll(".idx-row")
          .forEach((r) => r.classList.remove("idx-row-drag-over"));
        dragSrc = null;
      });
  ...
  _renumberAndSave(modal, bookId) {
    const rows = [
      ...modal.querySelectorAll(".idx-body .idx-row:not([hidden])"),
    ];
    rows.forEach((row, i) => {
      ...
      if (!filename) return;
      postJson(AppConfig.apiPath + "?action=doc.setOrder", {
        bookId,
        filename,
        order: i,
      }).catch(() => {});
  ...
  async _loadStates(bookId) {
    ...
    try {
      const data = await apiFetch(
        AppConfig.apiPath +
          `?action=state.get&bookId=${encodeURIComponent(bookId)}`
      );
      return data.states ?? {};
    } catch {
      return {};
  ...
  _saveState(bookId, filename, state) {
    if (!bookId) return;
    postJson(AppConfig.apiPath + "?action=state.set", {
      bookId,
      filename,
      state,
    }).catch(() => {});
  ...
  _filter(input, modalId) {
    ...
    document.querySelectorAll(`#${modalId} .idx-row`).forEach((row) => {
      const text =
        row.querySelector(".idx-row-title")?.textContent.toLowerCase() ?? "";
      row.hidden = !text.includes(q);
    });
    document
      .querySelectorAll(`#${modalId} .idx-act-header`)
      .forEach((header) => {
