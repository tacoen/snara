...
export class SnaraRouter {
  ...
  constructor() {
    ...
    window.navigate = (page, bookId = null, file = null) =>
      this.go(page, bookId, file);
  }

  registerRawSwitch(fn) {
    this._rawSwitch = fn;
  }
  registerRawLoad(fn) {
    this._rawLoad = fn;
  ...
  boot() {
    ...
    if (!params.p) {
      ...
      if (page) {
        this.go(page, bid || null, file || null);
        return;
      }
      this._title();
      return;
  ...
  _persist(page, bookId, file) {
    try {
      ...
      if (page === "editor" || page === "meta")
        localStorage.setItem("editor-filename", file ?? "");
    } catch {}
  }
