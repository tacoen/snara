...
export class SnaraGallery {
  ...
  async load() {
    ...
    try {
      const files = await apiFetch(
        `${AppConfig.apiPath}?action=gallery.list&bookId=${bookId}`
      );

      if (!files.length) {
        grid.innerHTML =
          '<p class="flist-empty">No media yet — drop files above.</p>';
        return;
      }
      ...
      grid.innerHTML = `<p class="flist-empty" style="color:var(--danger)">Error: ${esc(
        e.message
      )}</p>`;
    }
  }
  ...
  _makeCard(f, bookId) {
    ...
    const url = `${AppConfig.dataPath}/${bookId}/image/${encodeURIComponent(
      f.filename
    )}`;
    ...
    bar.innerHTML = `
      <span class="gallery-name" title="${esc(f.filename)}">${esc(
      f.filename
    )}</span>
  ...
  _openRename(card, filename, bookId) {
    ...
    input.addEventListener("input", () =>
      this._populateDatalist(datalist, input.value)
    );
    ...
    const doRename = async () => {
      ...
      try {
        ...
        const newUrl = `${
          AppConfig.dataPath
        }/${bookId}/image/${encodeURIComponent(newFilename)}`;
  ...
  async _fetchTerms(bookId) {
    try {
      const data = await apiFetch(
        `${AppConfig.apiPath}?action=gallery.autocomplete&bookId=${bookId}`
      );
    ...
    confirmDeleteBar(
      ...
      async () => {
        const res = await fetch(
          `${
            AppConfig.apiPath
          }?action=gallery.delete&bookId=${bookId}&filename=${encodeURIComponent(
            filename
          )}`,
          { method: "DELETE" }
        );
