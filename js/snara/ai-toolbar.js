export class SnaraAIToolbar {
  ...
  constructor(root, opts = {}) {
    ...
    this.#bindEntry =
      typeof opts.bindEntry === "function" ? opts.bindEntry : null;
  ...
  static async helper(fieldKey) {
    ...
    const prompt = SnaraAIToolbar.#builderPrompts.find(
      (p) => p.label === fieldKey
    );
    if (!prompt) {
    ...
    // 4. POST to ai.chat
    const endpoint =
      SnaraAIToolbar.instance?._endpoint ?? "api.php?action=ai.chat";
    let text = "";
    try {
    ...
    if (fieldKey === "characters") {
      const row = [...document.querySelectorAll(".meta-field")].find(
        (r) => r.dataset.key === "characters"
      );
      if (!row) return;
  ...
  async #loadPrompts() {
    try {
      ...
      if (!Array.isArray(data))
        throw new TypeError("preprompts.json must be an array");
      this.#prompts = data;
      this.#stampButtons();
    } catch (err) {
