...
function rebuildStyle(saved) {
  ...
  if (Object.keys(dv).length)
    parts.push(`html[data-theme="dark"] body {\n${alignedVarsBlock(dv)}\n}`);
  ...
  _render() {
    ...
    this.modal.innerHTML = `
      ...
      <div class="tabs">
        <button class="cfg-tab${
          t === "light" ? " active" : ""
        }" data-tab="light">☀ Light</button>
        <button class="cfg-tab${
          t === "dark" ? " active" : ""
        }" data-tab="dark">☾ Dark</button>
        <button class="cfg-tab${
          t === "root" ? " active" : ""
        }" data-tab="root">Spacing &amp; Type</button>
      </div>
    ...
    this.modal.querySelectorAll(".cfg-tab[data-tab]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        document.getElementById("pref-body").innerHTML = this._renderTabBody(
          this._activeTab
        );
        this._bindInputs();
    ...
    this.modal
      .querySelector("#pref-reset")
      .addEventListener("click", () => this._resetCurrentTab());
    this.modal
      .querySelector("#pref-export")
...
<div class="editor" contenteditable="true" data-placeholder="Write something…"></div>
  ...
  _renderTabBody(tab) {
    ...
    const formGroup = groups
      ...
      <section class="cfg-section pref-section">
        ...
        ${g.vars
          .map((v) =>
            this._renderRow(v, saved[v.name] ?? defaults[v.name] ?? "")
          )
          .join("")}
      </section>`
      )
  ...
  _renderRow({ name, label, type }, current) {
    ...
    if (type === "color") {
      ...
      return `
        <div class="cfg-row pref-row">
          <label class="cfg-label pref-var-label" title="${esc(name)}">${esc(
        label
      )}</label>
          <code class="pref-var-name">${esc(name)}</code>
          <input type="color" class="pref-color pref-input" data-var="${esc(
            name
          )}" data-peer="pt-${esc(safeId)}" value="${esc(hex)}">
          <input type="text" class="cfg-input pref-text pref-input" id="pt-${esc(
            safeId
          )}" data-var="${esc(name)}" data-peer="pc-${esc(
        safeId
      )}" data-canonical="1" value="${esc(
        current
    ...
    return `
      ...
      label
        ...
        <code class="pref-var-name">${esc(name)}</code>
        <input type="text" class="cfg-input pref-text pref-input" data-var="${esc(
          name
        )}" data-canonical="1" value="${esc(current)}" placeholder="e.g. 1rem">
      </div>`;
  }
    ...
    this.modal.querySelectorAll(".pref-input").forEach((input) => {
      ...
      if (input.type === "color") {
        input.addEventListener("input", () => {
          ...
          if (varName === "--fg-link")
            this._syncInputValue(input.value, "--selection");
        });
      } else {
        input.addEventListener("input", () => {
          ...
          if (val) {
            ...
            if (varName === "--fg-link")
              this._syncInputValue(val, "--selection");
          }
        });
      }
    });
  ...
  _snapshotTab() {
    ...
    const out = {};
    this.modal
      .querySelectorAll(".pref-input[data-canonical]")
      .forEach((inp) => {
