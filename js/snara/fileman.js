...
export class SnaraFileMan {
  ...
  _bindStatic() {
    ...
    if (this._dz) {
      this._dz.addEventListener("dragover", (e) => {
        e.preventDefault();
        this._dz.classList.add("drag");
      });
      this._dz.addEventListener("dragleave", () =>
  ...
  async _fetchList() {
    ...
    try {
      const data = await apiFetch(
        `${AppConfig.apiPath}?action=fileman.list&bookId=${encodeURIComponent(
          this._bookId
        )}`
      );
  ...
  _buildRow(f) {
    ...
    const dlUrl = `${AppConfig.dataPath}/${
      this._bookId
    }/files/${encodeURIComponent(f.filename)}`;
    ...
    li.innerHTML = `
      ...
      <span class="fbadge text-mono" style="opacity:.6">${fmtDate(
        f.mtime
      )}</span>
      <div class="ftools">
        ${
          preview
            ? `
        <button class="ftool fm-preview-btn" title="Preview" data-filename="${esc(
          f.filename
        )}">
          <i data-icon="eye"></i>
        </button>`
        ...
        ${
          editable
            ? `
        <button class="ftool fm-edit-btn" title="Edit" data-filename="${esc(
          f.filename
        )}">
        ...
        <button class="ftool fm-rename-btn" title="Rename" data-filename="${esc(
          f.filename
        )}">
          <i data-icon="pencil"></i>
        </button>
        <button class="ftool erase-btn fm-delete-btn" title="Delete" data-filename="${esc(
          f.filename
        )}">
          <i data-icon="trash"></i>
        </button>
  ...
  _openPreview(filename, type, url) {
    ...
    dlBtn.addEventListener("mouseenter", () => {
      ...
      dlBtn.style.borderColor = "rgba(255,255,255,.5)";
    });
    dlBtn.addEventListener("mouseleave", () => {
      dlBtn.style.color = "rgba(255,255,255,.6)";
      dlBtn.style.borderColor = "rgba(255,255,255,.2)";
    });
  ...
  async _uploadFiles(files) {
    ...
    label.style.cssText =
      "font-family:var(--font-mono);font-size:var(--f-xs);color:var(--fg-muted)";
    ...
    fill.style.cssText =
      "height:100%;width:0%;background:var(--primary);border-radius:9999px;transition:width .2s ease";
    ...
    for (const file of list) {
      ...
      try {
        const json = await apiFetch(
          `${
            AppConfig.apiPath
          }?action=fileman.upload&bookId=${encodeURIComponent(this._bookId)}`,
          { method: "POST", body: fd }
        );
        const existing = this._files.findIndex(
          (f) => f.filename === json.file.filename
        );
  ...
  _deleteFile(filename, li) {
    confirmDeleteBar(`Delete "${filename}"?`, async () => {
      await apiFetch(
        `${AppConfig.apiPath}?action=fileman.delete&bookId=${encodeURIComponent(
          this._bookId
  ...
  _openRename(li, filename) {
    ...
    box.innerHTML = `
      <input class="fm-rename-input" value="${esc(
        basename
    ...
    const doRename = async () => {
      ...
      if (!newBase || newBase + ext === filename) {
        restore();
        return;
      }
      ...
      try {
        const json = await postJson(
          `${
            AppConfig.apiPath
          }?action=fileman.rename&bookId=${encodeURIComponent(this._bookId)}`,
        ...
        li.querySelectorAll("[data-filename]").forEach(
          (el) => (el.dataset.filename = newFilename)
        );
        restore();
      } catch (e) {
  ...
  async _openEditor(filename) {
    ...
    if (this._dirty && this._editFile) {
      if (!confirm(`Discard unsaved changes to "${this._editFile.filename}"?`))
        return;
    }
  ...
  _renderEditorTopActions(filename) {
    ...
    el.querySelector(".fm-save-btn").addEventListener("click", () =>
      this._saveEdit()
    );
    icx.delayreplace("#files-topbar-actions [data-icon]");
  }
