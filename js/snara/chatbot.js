...
export class SnaraChat {
  ...
  constructor(selector) {
    ...
    if (!(this._root instanceof HTMLElement)) {
      console.error(
        `${TAG} Root element not found for selector: "${selector}"`
      );
      return;
    }
    ...
    const missing = ["_window", "_input", "_sendBtn"].filter((k) => !this[k]);
    if (missing.length) {
  ...
  _handleDelegate(e) {
    ...
    if (!btn) return;
    const textEl = btn
      .closest(".chatbot__message")
      ?.querySelector(".chatbot__text");
  ...
  _handleTocClick(e) {
    ...
    e.preventDefault();
    this._aside
      .querySelectorAll(".chat-side__item--active")
  ...
  async _apiRequest(method, body = null) {
    const ACTION_MAP = {
      GET: "chatlog.get",
      POST: "chatlog.save",
      DELETE: "chatlog.clear",
    };
    const url = `${AppConfig.apiPath}?action=${
      ACTION_MAP[method]
    }&bookId=${encodeURIComponent(AppConfig.activeBookId)}`;
    const opts = {
      method,
