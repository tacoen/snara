...
export class SnaraUI {
  ...
  constructor() {
    ...
    this._initTheme();
    const initial =
      document.querySelector(".tabmenu li.active")?.dataset.tab || "editor";
  ...
  focusEntry(div) {
    ...
    const currentClass = SnaraEditor.CLASSES.find((c) =>
      div.classList.contains(c)
    );
    this.popup
      .querySelectorAll(".pop-pill")
      .forEach((pill) => pill.classList.remove("sel"));
    if (currentClass) {
      this.popup
        .querySelector(`.pop-pill.${currentClass}`)
        ?.classList.add("sel");
    }
  }
  ...
  async saveDocument() {
    ...
    // Detect in-browser rename: compare live span value against the
    // server-side filename stored on the article element.
    const liveFilename = fnEl.innerText.trim() || "untitled";
    const storedFilename = this.article?.dataset.filename || "";
    let filename = liveFilename;

    if (
      storedFilename &&
      liveFilename !== storedFilename &&
      bookId
    ) {
      try {
        const result = await postJson(
          AppConfig.apiPath + "?action=doc.rename",
          { from: storedFilename, to: liveFilename, bookId }
        );
        // Use the server-sanitized name for the rest of the save
        filename = result.filename ?? liveFilename;
        // Sync DOM and router state to the canonical new name
        fnEl.innerText = filename;
        this.article.dataset.filename = filename;
        try {
          localStorage.setItem("editor-filename", filename);
        } catch {}
      } catch (err) {
        // Rename failed — restore the span and abort
        fnEl.innerText = storedFilename;
    ...
    this.entriesEl.querySelectorAll(".entry").forEach((div) => {
      const cls =
        AppConfig.classes.find((c) => div.classList.contains(c)) || "beat";
      const html =
    ...
    document.querySelectorAll(".meta-field").forEach((row) => {
      const key =
        row.dataset.key || row.querySelector(".field-key")?.innerText.trim();
      if (!key) return;
      ...
      if (pillContainer) {
        const vals = [...pillContainer.querySelectorAll(".field-pill")]
          .map(
            (p) =>
              (
                p.querySelector(".pill-text") ?? p.firstChild
              )?.textContent.trim() ?? ""
          )
          .filter(Boolean);
        meta[key] = vals.join(", ");
    ...
    try {
      ...
      this.entriesEl
        .querySelectorAll(".entry[data-advice]")
        .forEach((el) => delete el.dataset.advice);
    } catch (err) {
      if (btn) {
        btn.classList.remove("saving");
  ...
  async loadDocument(bookId, filename, tab = "editor") {
    const url =
      AppConfig.apiPath +
      `?action=doc.get&filename=${encodeURIComponent(
        filename
  ...
  _renderDocument(data, tab = "editor") {
    ...
    if (SnaraEditor.instance?._isDirty()) {
      const ok = confirm(
        "You have unsaved edits. Discard and load new document?"
      );
      if (!ok) return;
    ...
    // Sync the act heading with what chapters.json has as the canonical act name.
    // If the h1 text in the .entry.act div differs, overwrite it and mark dirty.
    this._syncActHeading(data.filename);
    ...
    if (metaFields) {
      ...
      const defaultFields = AppDefaults.metaFields ?? [
        "characters",
        "settings",
        "prompts",
      ];
  ...
  // Sync the act h1 heading in the rendered doc against cache/chapters.json.
  // Runs async after render — failure is silent (cache may not exist yet).
  async _syncActHeading(filename) {
    if (!filename || !AppConfig.activeBookId) return;

    const actDiv = this.entriesEl.querySelector(".entry.act");
    if (!actDiv) return;

    const h1 = actDiv.querySelector("h1");
    if (!h1) return;

    try {
      const url = `${AppConfig.dataPath}/${AppConfig.activeBookId}/cache/chapters.json`;
      const res = await fetch(url);
      if (!res.ok) return;
      const cache = await res.json();

      const chapters = Array.isArray(cache.chapters) ? cache.chapters : [];
      const entry = chapters.find((ch) => ch.filename === filename);
      if (!entry || !entry.act) return;

      const canonical = entry.act.trim();
      if (!canonical || canonical === h1.innerText.trim()) return;

      // Act name in cache differs — overwrite and mark entry dirty for next save.
      h1.innerText = canonical;
      actDiv.dataset.advice = "Act name synced from index";
    } catch {
      // cache missing or malformed — skip silently
    }
  }
  ...
  _buildMetaRow(key, val) {
    ...
    if (isPill) {
      row.dataset.key = key;
      const pills = String(val || "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      row.innerHTML = `
        <span class="field-key" style="cursor:pointer" onclick="SnaraAIToolbar.helper('${esc(
          key
        )}')" title="Auto-populate from article">${esc(key)}</span>
        <span class="field-sep">:</span>
        <div class="field-pills">
          ${pills
            .map(
              (p) =>
                `<span class="field-pill"><span class="pill-text">${esc(
                  p
                )}</span><button class="pill-remove" data-val="${esc(
                  p
    ...
    row.innerHTML = `
      <span class="field-key" contenteditable spellcheck="false">${esc(
        key
      )}</span>
      <span class="field-sep">:</span>
      <span class="field-val" contenteditable spellcheck="false">${esc(
        val
      )}</span>
      <button class="field-remove" title="Remove field">x</button>
    `;
  ...
  _bindPillSearch(pillEl, value) {
    ...
    pillText.addEventListener("click", (e) => {
      ...
      const url = `${
        AppConfig.apiPath
      }?action=query.search&bookId=${encodeURIComponent(
        bid
      ...
      fetch(url)
        ...
        .then((data) => {
          if (data.error) {
            contentArea.innerHTML = `<p class='danger'>${data.error}</p>`;
            return;
          }
          ...
          contentArea
            .querySelector(".build-results-btn")
  ...
  _buildResults(bid, query, contentArea) {
    const unchecked = [
      ...contentArea.querySelectorAll(".query-check:not(:checked)"),
    ]
    ...
    fetch(`${AppConfig.apiPath}?action=query.build`, {
      ...
      .then((data) => {
        if (data.error) {
          console.error("[query.build]", data.error);
          return;
        }
        ...
        if (h4)
          h4.textContent = `${remaining} paragraph(s) kept for "${query}"`;
      })
