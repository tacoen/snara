...
export class SnaraFiles extends SnaraComponent {
  ...
  async _uploadFiles(files, type) {
    ...
    if (!bookId) {
      alert("No active book — open a book first.");
      return;
    }
  ...
  async _loadImpList() {
    ...
    try {
      const files = await apiFetch(
        `${AppConfig.apiPath}?action=import.list&bookId=${bookId}`
      );
      ...
      ul.addEventListener("click", this._impListClick.bind(this), {
        once: true,
      });
  ...
  _impListClick(e) {
    ...
    if (!btn) {
      document
        .getElementById("files-imp-list")
        ?.addEventListener("click", this._impListClick.bind(this), {
          once: true,
        });
      return;
    }
  ...
  async _openPreview(filename) {
    ...
    modal.innerHTML = `
      ${_modalHeader(
        `<i data-icon="file-text"></i> Preview — ${esc(filename)}`
      )}
    ...
    try {
      const res = await fetch(
        `${
          AppConfig.apiPath
        }?action=import.read&bookId=${bookId}&filename=${encodeURIComponent(
          filename
        )}`
      );
    ...
    modal.innerHTML = `
      ...
      <div class="modal-body" style="padding:var(--s-md);overflow-y:auto;display:flex;flex-direction:column;gap:var(--s-sm)">
        <div class="cfg-row">
          ...
          <input class="cfg-input" id="imp-filename" value="${esc(
            defaultFilename
          )}"
          ...
          ${blocks
            ...
            <div class="imp-block" style="border:1px solid var(--border);border-radius:6px;overflow:hidden">
              <div style="display:flex;align-items:center;gap:8px;padding:6px 10px;background:var(--bg-alt);border-bottom:1px solid var(--border)">
                <span class="fbadge imp-cls-badge" data-idx="${bi}" title="Click to cycle class" style="cursor:pointer">${esc(
                b.cls
              )}</span>
              ...
              <pre style="margin:0;padding:8px 10px;font-size:11px;font-family:var(--font-mono);white-space:pre-wrap;color:var(--fg-muted);max-height:100px;overflow-y:auto;background:var(--bg-main)">${esc(
                b.md
              )}</pre>
            </div>`
            )
  ...
  _showPreviewError(modal, msg) {
    modal.innerHTML = `
      ...
      <div class="modal-body" style="padding:var(--s-lg);color:var(--danger);font-size:var(--f-xs)">${esc(
        msg
      )}</div>
  ...
  async _confirmImport(modal, srcFilename) {
    ...
    const filename =
      document.getElementById("imp-filename")?.value.trim() ||
      srcFilename
        .replace(/\.[^.]+$/, "")
        .replace(/\s+/g, "-")
        .toLowerCase();
    ...
    try {
      ...
      try {
        await fetch(
          `${
            AppConfig.apiPath
          }?action=import.delete&bookId=${bookId}&filename=${encodeURIComponent(
            srcFilename
          )}`,
          { method: "DELETE" }
        );
      ...
      window.dispatchEvent(
        new CustomEvent("chapteradded", { detail: { bookId, filename } })
      );
    } catch (e) {
      btn.disabled = false;
      ...
      btn.insertAdjacentHTML(
        "beforebegin",
        `<span class="imp-err" style="color:var(--danger);font-size:11px">Error: ${esc(
          e.message
        )}</span>`
      );
    }
  ...
  async _loadCacheList() {
    ...
    try {
      const data = await apiFetch(
        `${AppConfig.apiPath}?action=cache.list&bookId=${bookId}`
      );
      ...
      if (!Array.isArray(data)) {
        listSetState(
          ul,
          "error",
          `API error: ${data?.error ?? "unexpected response"}`
        );
        return;
      }
          ...
          <li class="flist-item" style="flex-direction:column;align-items:flex-start;gap:4px;padding:var(--s-sm) var(--s-md)">
            <div style="display:flex;align-items:center;gap:8px;width:100%">
              ...
              <span class="fname" style="flex:1">
                <span style="font-size:10px;color:var(--fg-muted);font-family:monospace">cache/</span>${esc(
                  f.name
                )}
              </span>
              ${staleWarning}
  ...
  async rebuildCache() {
    ...
    const btn = document.querySelector(
      "#fpanel-cache .fpanel-footer .btn-mini"
    );
    ...
    try {
      const json = await postJson(
        `${AppConfig.apiPath}?action=cache.rebuild&bookId=${bookId}`,
        {}
      );
      ...
      prog.innerHTML = `<span style="color:var(--danger)">Rebuild failed: ${esc(
        e.message
      )}</span>`;
    } finally {
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i data-icon="refresh"></i> Rebuild cache';
      }
  ...
  _updateDeleteBar(sec) {
    const barId =
      sec === "import" ? "files-delete-bar" : `files-delete-bar-${sec}`;
